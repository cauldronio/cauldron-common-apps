...
from .models import IParseSPDXArchived, IParseSPDX, SPDXUserFile
...
@admin.register(IParseSPDX)
class IntentionAdmin(admin.ModelAdmin):
    list_display = ('id', 'spdx_file', 'created', 'job', user_name, previous_count)
    search_fields = ('id',)
    list_filter = ('created', RunningInAWorker)
    ordering = ('created', )


@admin.register(IParseSPDXArchived)
class ArchivedIntentionAdmin(admin.ModelAdmin):
    list_display = ('id', 'user_file', 'created', 'completed', user_name, 'status', 'arch_job', 'logs')
...
@admin.register(SPDXUserFile)
class SPDXUserFileAdmin(admin.ModelAdmin):
    list_display = ('id', 'created_at', 'uploaded_by_name', 'name', 'location')
